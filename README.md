...
## 2. tools/ (工具目录)
├── litex_soc_gen.py (SoC生成器)
├── litex_sim.py (仿真工具)
├── litex_term.py (终端工具)
├── litex_server.py (服务器)
├── litex_client.py (客户端)
├── litex_json2dts_linux.py (Linux设备树生成)
├── litex_json2dts_zephyr.py (Zephyr设备树生成)
├── litex_json2renode.py (Renode设备树生成)
├── litex_read_verilog.py (Verilog读取)
├── litex_periph_gen.py (外设生成器)
└── litex_contributors.py (贡献者信息)
